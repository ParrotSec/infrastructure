...
var settings = {
	...
	time_ul_max: 25, // max duration of upload test in seconds
	...
	time_ulGraceTime: 5, //time to wait in seconds before actually measuring ul speed (wait for buffers to fill)
	...
	xhr_dlUseBlob: true, // if set to true, it reduces ram usage but uses the hard drive (useful with large garbagePhp_chunkSize and/or high xhr_dlMultistream)
	...
	ping_allowPerformanceApi: false, // if enabled, the ping test will attempt to calculate the ping more precisely using the Performance API. Currently works perfectly in Chrome, badly in Edge, and not at all in Firefox. If Performance API is not supported or the result is obviously wrong, a fallback is provided.
